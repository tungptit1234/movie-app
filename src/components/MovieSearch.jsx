    ...
    <div className="my-10 px-10 max-w-full">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
        {data.map((item) => (
          <div
            key={item.id}
            className="bg-cover bg-no-repeat bg-center w-[200px] h-[300px] relative hover:scale-110 transition-transform duration-500 ease-in-out cursor-pointer border border-gray-700"
            style={{
              // Kiểm tra: Nếu có poster_path thì dùng ảnh của API, nếu không có thì dùng ảnh xám có chữ "No Poster"
              backgroundImage: `url(${
                item.poster_path
                  ? `${import.meta.env.VITE_IMG_URL}${item.poster_path}`
                  : "https://placehold.co/200x300/333333/FFFFFF.png?text=No+Poster"
              })`,
            }}
          ...
          >
            ...
            <div className="relative p-4 flex flex-col items-center justify-end h-full">
              <h3 className="text-md uppercase text-white font-bold text-center">
